...
export const BlogCard = ({
  ...
  return (
    <Link to={`/blog/${id}`}><div className="border-b p-4 grid grid-cols-12 max-w-3xl gap-4 mt-2">
    ...
    <div className="col-span-5 sm:col-span-3">
      <div>
      <img src={`https://source.unsplash.com/random/?${imgTag}`} alt="" className="p-1 h-32 w-40 sm:h-40 sm:w-48"/>
      </div>
...
export function Avatar({ name, size = "small" }: { name: string, size?:"big" | "small" }) {
  return (
    <div className={`relative inline-flex items-center justify-center overflow-hidden bg-gray-300 rounded-full ${size==="big"? "w-10 h-10":"w-6 h-6"}`}>
      <span className={`font-normal text-gray-600 ${size==="big"? "text-xl":"text-base"}`}>{name[0]}</span>
    </div>
  );
}
